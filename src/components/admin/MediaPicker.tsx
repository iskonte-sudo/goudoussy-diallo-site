          ...
          <div className="card-photo-placeholder h-20 w-20 flex-none overflow-hidden rounded-md text-[0.6rem]">
            {selectedUrl ? (
              mode === "video" ? (
                ...
                <Image src={selectedUrl} alt="" width={80} height={80} className="h-full w-full object-cover" />
              )
            ) : (
              "Aucun média"
            )}
                ...
                <div className="grid flex-1 grid-cols-4 gap-3 overflow-y-auto">
                  {loading ? (
                    ...
                    items.map((item) => (
                      ...
                      >
                        {item.type === "IMAGE" ? (
                          <Image src={item.url} alt={item.altText ?? ""} width={150} height={150} className="h-full w-full object-cover" />
                        ) : (
                          <div className="flex h-full w-full flex-col items-center justify-center gap-1 bg-navy text-white">
                            {item.thumbnailUrl ? (
                              <Image src={item.thumbnailUrl} alt="" width={150} height={150} className="h-full w-full object-cover opacity-70" />
                            ) : (
