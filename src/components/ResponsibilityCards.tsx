...
export default function ResponsibilityCards({ items }: { items: Responsibility[] }) {
  return (
      ...
      {items.map((r) => (
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(0deg, rgba(6,10,22,0.92) 0%, rgba(6,10,22,0.35) 55%, rgba(6,10,22,0.1) 100%)"
            }}
          />
          <div
            className="absolute left-0 right-0 top-0 h-[5px]"
            style={{ background: DOMAIN_COLOR[r.domain] ?? "#CE1126" }}
          />
          {!r.imageUrl && (
          ...
          <div className="absolute inset-x-0 bottom-0 p-6 text-white">
            ...
            <h3 className="mb-2.5 text-xl font-bold leading-snug">{r.title}</h3>
            <p className="text-sm text-[#D6D9E0]">{r.description}</p>
          </div>
        </div>
      ))}
    </div>
